 ...
 *
 * Covered behaviors:
 * - depends only on the HybridSearch and ContextAssembler ports, verified
 *   via a static source-scan for a forbidden concrete-adapter/lower-level
 *   port import
 * - execute() calls HybridSearch.search before ContextAssembler.assemble
 *   (verified via a shared call-order log across both counting test
 *   doubles)
 * - maps RetrieveGroundingContextInput onto HybridSearch's RetrievalInput
 ...
 *   direct HybridSearch.search -> ContextAssembler.assemble call sequence
 * - rejects invalid workspaceId/query/retrievalLimit/maxCharacters input
 *   (and a non-object input) before ever calling HybridSearch or
 *   ContextAssembler
 */
export const RETRIEVE_GROUNDING_CONTEXT_USE_CASE_UNIT_CASES = [
  "depends_only_on_hybrid_search_and_context_assembler_ports",
  "execute_calls_hybrid_search_before_context_assembler",
