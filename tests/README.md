...
| Executable checks | `pnpm validate:repository`, `pnpm validate:repository:source`, `pnpm validate:repository:chunk`, `pnpm validate:application`, `pnpm validate:pipeline:connector`, `pnpm validate:pipeline:sync`, `pnpm validate:pipeline:chunk-document`, `pnpm validate:pipeline:rechunk-source`, `pnpm validate:embedding:chunker` |
