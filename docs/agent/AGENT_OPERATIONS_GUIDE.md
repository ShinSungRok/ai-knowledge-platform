...
> Project 2 — Cursor Agent Operations Guide
...
이 문서는 **AI Knowledge Platform(Project 2)** 개발에 사용하는 Cursor Agent Skills의 역할, 설치 위치, 호출 방법, 운영 규칙을 정의한다.

이 문서는 프로젝트의 최상위 개발 지시서가 아니며, **Skill 선택·설치·호출·문제 해결이 필요할 때만 참조하는 운영 매뉴얼**이다.

프로젝트 목적, 아키텍처 원칙, 개발 절차, Validation, Commit 규칙은 별도의 `PROJECT02_INSTRUCTIONS.md`를 따른다.
...
- `PROJECT02_INSTRUCTIONS.md`: 무엇을 만들고 어떤 원칙으로 개발할지 정의
- `AGENT_OPERATIONS_GUIDE.md`: 어떤 Agent Skill을 언제 어떻게 사용할지 정의
- 개별 Task Prompt: 현재 작업의 구체적인 범위와 완료 조건 정의

일반 작업에서는 `PROJECT02_INSTRUCTIONS.md`만 상시 적용한다. Skill 사용법이 필요할 때만 이 문서를 읽는다.

Skill 사용 결과가 프로젝트 지시서와 충돌하면 항상 `PROJECT02_INSTRUCTIONS.md`를 우선한다.
...
```text
프로젝트 규칙    → PROJECT02_INSTRUCTIONS.md
...
```text
docs/agent/PROJECT02_INSTRUCTIONS.md를 먼저 읽어라.
