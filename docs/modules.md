...
| `llmops` | Project 4 Enterprise LLMOps boundary. Experiment / Run Tracking: `ExperimentRunStore` + `InMemoryExperimentRunStore`. Prompt & Model Registry: `PromptRegistry` / `ModelRegistry` with InMemory adapters. Evaluation Gates / Regression Harness: `EvaluationGateEvaluator` / `RegressionHarness` with Default adapters (numeric metrics; no LLM-as-judge). Deployment / Serving Configuration: `ServingConfigStore` with soft-link `promptVersionId` / `modelVersionId` / optional `gateId` (store does not call registries or gates). Soft link only: run `params` / `metrics` may link to registry or evaluation aggregates. Distinct from Project 2 `JobStore` and Project 3 `WorkflowRunId`. LLMOps Observability and HTTP serving remain deferred; does not bind `ai` LanguageModelProvider. |
