...
| `workflow` | Multi-Agent role/identity + Workflow Orchestrator boundary (Project 3). `WorkflowAgentId`, `WorkflowAgentRole`, `WorkflowAgentDescriptor`, identity-only `WorkflowAgent`, and `WorkflowAgentRegistry` / `InMemoryWorkflowAgentRegistry` define participants. `WorkflowGoal` / `WorkflowPlan` / `WorkflowPlanStep` / `WorkflowStepResult` / `WorkflowRunResult` plus `WorkflowPlanner` / `WorkflowOrchestrator` ports define goal→plan→invoke→result without explicit Handoff types or Shared Workflow Memory. Distinct from Project 2 single-agent `AgentRole` / `AgentOrchestrator` under `agent`. |
