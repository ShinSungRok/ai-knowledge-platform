...
| `composition` | Composition root — wires concrete adapters. `createInMemoryKnowledgeComposition` assembles in-memory/fake adapters through the cited-answer use-case chain, MCP registry/handler, and `KnowledgeRuntime` (LLM defaults to Fake; optional HTTP LLM). Operations/listening servers expose `POST /mcp` JSON-RPC plus cited-answer with Bearer AuthN. SQL composition paths and OpenSearch remain separate deferred tracks. |
...
| `observability` | Logging, metrics, and OTLP/HTTP export boundary. `InMemoryLogger`/`InMemoryMetrics` remain the default sinks. `OtlpLogsExporter`/`OtlpMetricsExporter` plus `ExportingLogger`/`ExportingMetrics` (sync `log` + async `flush`) and `FetchOtlpHttpTransport` enable optional collector export without official OpenTelemetry SDK. |
