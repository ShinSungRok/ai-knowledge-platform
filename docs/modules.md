...
| `llmops` | Project 4 Enterprise LLMOps boundary. Experiment / Run Tracking: `ExperimentId` / `ExperimentRunId`, `ExperimentRunStatus`, `ExperimentRunRecord`, and `ExperimentRunStore` port. `InMemoryExperimentRunStore` (`DefaultExperimentRunStore` alias) keys by `(workspaceId, runId)`, lists by experiment ordered by `startedAtUnixMs` then id, enforces pending→running→completed\|failed transitions, workspace isolation, and defensive copies. Distinct from Project 2 `JobStore` and Project 3 `WorkflowRunId`. Prompt/Model Registry, Evaluation Gates, Serving, and LLMOps Observability remain deferred. |
