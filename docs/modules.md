...
and defensive copies. Processor/handlers are later tasks.
...
| `jobs` | Background Job boundary for long-running Sync/Reindex pipeline work. `JobType` (`"sync_knowledge_source" \| "reindex_knowledge_source"`), `JobStatus` (`"pending" \| "running" \| "completed" \| "failed"`), `JobRecord`, and the `JobStore` / `JobHandler` / `JobProcessor` ports define enqueue/process contracts without real workers, cron, or network brokers. `InMemoryJobStore` assigns 1-based workspace `sequence`, deterministic `id` (`${workspaceId}:${sequence}`), pending enqueue, get/list/save with workspace isolation and defensive copies. Processor/handlers are later tasks. |
