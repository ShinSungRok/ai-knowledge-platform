...
| `workflow` | Multi-Agent role/identity contract (Project 3). `WorkflowAgentId`, `WorkflowAgentRole` (`"coordinator" \| "researcher" \| "synthesizer" \| "critic" \| "executor"`), `WorkflowAgentDescriptor`, and the `WorkflowAgent` port define Multi-Agent participants without inventing a run/orchestrator API. Distinct from Project 2 single-agent `AgentRole` (`planner`/`executor`/`reviewer`) under `agent`. `WorkflowAgentRegistry` + `InMemoryWorkflowAgentRegistry` register/look up agents by id and role (sync void; duplicate id and empty id/displayName throw; defensive list copies). WorkflowOrchestrator, handoff, shared workflow memory, and multi-agent evaluation remain deferred. |
