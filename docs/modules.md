...
| `embedding` | Chunking, embedding, and vector indexing. `VectorIndex` with `InMemoryVectorIndex` / `SqlVectorIndex` (default). `OpenSearchHttpTransport` / `loadOpenSearchClientConfig` define an OpenSearch REST boundary for a later VectorIndex adapter (official OpenSearch JS SDK deferred). |
