...
| `api` | Controllers and request/response DTOs. `HealthController` (`GET /health`) and `CitedGroundedAnswerController` (`POST /workspaces/{workspaceId}/cited-answers`) depend only on `KnowledgeRuntime`. `createKnowledgeHttpRouter` registers both on a framework-independent `HttpRouter`. |
| `http` | Framework-independent HTTP abstraction: `HttpMethod`/`HttpRequest`/`HttpResponse`, `HttpHandler`/`HttpRouter` ports, and `DefaultHttpRouter` (exact method+path match; JSON 404 on miss). No Express/Fastify/node:http listen. |
