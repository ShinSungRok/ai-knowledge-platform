...
| `llmops` | Project 4 Enterprise LLMOps boundary. Experiment / Run Tracking: `ExperimentRunStore` + `InMemoryExperimentRunStore`. Prompt & Model Registry: `PromptRegistry` / `ModelRegistry` with `InMemoryPromptRegistry` / `InMemoryModelRegistry` (`Default*` aliases) — duplicate id/version reject, workspace isolation, list by name/version then id, defensive copies. Soft link only: run `params` may later store `promptVersionId` / `modelVersionId`. Distinct from Project 2 `JobStore` and Project 3 `WorkflowRunId`. Gates, Serving, and LLMOps Observability remain deferred; does not bind `ai` LanguageModelProvider. |
