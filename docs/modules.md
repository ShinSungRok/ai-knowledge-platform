...
| `workflow` | Multi-Agent role/identity + Workflow Orchestrator (Project 3). `WorkflowAgent` remains identity-only; `WorkflowAgentRegistry` / `InMemoryWorkflowAgentRegistry` register participants. `WorkflowGoal` / `WorkflowPlan` / step+run results plus `WorkflowPlanner` / `WorkflowAgentInvoker` / `WorkflowOrchestrator` define goal→plan→invoke→result. `DeterministicWorkflowPlanner` builds a fixed-role priority plan (all steps get `goal.objective`). `DefaultWorkflowOrchestrator` composes planner+registry+invoker, stops on first failure (no skipped entries; v1 any failure → `failed`). Explicit Handoff and Shared Workflow Memory remain deferred. Distinct from Project 2 single-agent `AgentRole` / `AgentOrchestrator`. |
...
app/knowledge/
  ...
  ai/ mcp/ tools/ agent/ workflow/ memory/ jobs/ application/
  api/ http/ server/
  composition/ config/
