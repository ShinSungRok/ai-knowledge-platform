...
| `observability` | Logging, metrics, and OTLP/HTTP export boundary. `Logger`/`LogEvent`/`LogLevel` and `Metrics`/`MetricPoint` with `InMemoryLogger`/`InMemoryMetrics`. `OtlpHttpTransport` / `OtlpExporterConfig` / `loadOtlpExporterConfig` define optional collector export without official OpenTelemetry SDK. |
