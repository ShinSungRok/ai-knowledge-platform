...
| `infra` | Infrastructure edge for Source-of-Truth persistence and Docker scaffolding. `SqlGateway` / `SqlParameter` / `SqlQueryResult` define the SQL execute contract. `knowledgeSchemaSql` + `applyKnowledgeSchema` apply CREATE TABLE IF NOT EXISTS for sources/documents/chunks. `InMemorySqlGateway` validates repository SQL (DDL no-op) without a live DB. `PostgresSqlGateway` adapts a `PostgresPool` (`pg.Pool`-compatible); default validate does not open connections. Local Docker helpers via `pnpm infra:config`. |
