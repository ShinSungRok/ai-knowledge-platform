...
| `persistence` | Concrete adapters (`DefaultInMemoryRepository`, `DefaultInMemoryKnowledgeSourceRepository`, `DefaultInMemoryDocumentChunkRepository`, `SqlKnowledgeDocumentRepository` / `SqlKnowledgeSourceRepository` / `SqlDocumentChunkRepository` over `SqlGateway`). Real `pg` driver still deferred. |
...
| `embedding` | Chunking, embedding, and vector indexing ports/adapters. `ChunkingService` port + `FixedSizeDocumentChunker` deterministic, fixed-size adapter split a `KnowledgeDocument` into ordered `DocumentChunk`s. `EmbeddingProvider` port + `FakeEmbeddingProvider` deterministic adapter turn text into a fixed-`EMBEDDING_VECTOR_DIMENSION` (8) vector. `VectorIndex` port + `InMemoryVectorIndex` / `SqlVectorIndex` (SqlGateway, `embedding_vectors`) upsert/find/delete an `EmbeddingVector` by `(workspaceId, chunkId)`, and rank vectors within a workspace via `findNearest` (cosine similarity, `ScoredEmbeddingVector[]`). OpenSearch client remains deferred. Chunk hydration and hybrid search are handled by `retrieval`/`search`. |
...
| `mcp` | Transport-independent MCP tool registry plus JSON-RPC handler boundary. `McpTool` / `McpToolRegistry` expose capabilities without Domain/RAG duplication. `McpJsonRpcRequest` / `McpJsonRpcResponse` / `McpJsonRpcHandler` and `tools/list`·`tools/call` method constants define a dependency-free JSON-RPC subset (no official MCP SDK). `GenerateCitedGroundedAnswerMcpTool` and `DefaultMcpToolRegistry` remain the in-process tool surface. |
...
| `api` | Controllers and request/response DTOs. `HealthController` (`GET /health`, no auth) and `CitedGroundedAnswerController` (`POST /workspaces/{workspaceId}/cited-answers`) use `KnowledgeRuntime` plus Bearer AuthN (`HttpBearerGuard`) and workspace AuthZ (`WorkspaceAuthorizer`). `createKnowledgeHttpRouter(runtime, bearerGuard, workspaceAuthorizer)` registers both; cited-answer requires `Authorization: Bearer <api-key>`. |
...
| `composition` | Composition root — wires concrete adapters. `createInMemoryKnowledgeComposition` assembles in-memory/fake adapters through the cited-answer use-case chain and exposes `KnowledgeRuntime` (LLM defaults to Fake; optional `llm: { type: "http", config }` uses `HttpLanguageModelProvider` + `FetchLlmHttpTransport`). `createSqlDocumentKnowledgeComposition` / `createSqlKnowledgeComposition` / `createPostgresKnowledgeComposition` provide SQL paths. `createInMemoryKnowledgeServer` and `createOperationsKnowledgeServer` are dispatch-only; `createListeningOperationsServer` adds `NodeHttpListener` TCP listen (default 127.0.0.1:0). Operations/listening factories require `apiKeys` for Bearer AuthN and keep Fake LLM by default. Full agent/memory/jobs/MCP platform wiring and OpenSearch remain deferred. |
...
| `security` | Authentication and workspace authorization. `Authenticator` / `AuthPrincipal` define AuthN; `ApiKeyAuthenticator` maps static API keys; `HttpBearerGuard` parses `Authorization: Bearer`. `WorkspaceAuthorizer`/`DefaultWorkspaceAuthorizer` and `HttpWorkspaceGuard` (`x-workspace-id`) enforce AuthZ. Health stays unauthenticated. JWT/OIDC SDKs remain deferred. |
| `infra` | Infrastructure edge for Source-of-Truth persistence and Docker scaffolding. `SqlGateway` / `SqlParameter` / `SqlQueryResult` define the SQL execute contract. `knowledgeSchemaSql` + `applyKnowledgeSchema` apply CREATE TABLE IF NOT EXISTS for sources/documents/chunks plus `embedding_vectors` (rebuildable search index; OpenSearch deferred). `InMemorySqlGateway` validates repository SQL (DDL no-op) without a live DB. `PostgresSqlGateway` adapts a `PostgresPool` (`pg.Pool`-compatible); default validate does not open connections. Local Docker helpers via `pnpm infra:config`. |
