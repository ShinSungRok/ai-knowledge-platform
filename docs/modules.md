...
| `workflow` | Multi-Agent role/identity + Workflow Orchestrator + Agent Handoff (Project 3). `WorkflowAgent` remains identity-only; registry registers participants. `WorkflowGoal` / plan / invoker / orchestrator define goal→plan→invoke→result. `WorkflowHandoff` + `DefaultWorkflowHandoffBuilder` pass prior output into later steps (sequential or coordinator-delegation); step 0 keeps planned objective. Shared Workflow Memory and Multi-Agent Evaluation remain deferred. Distinct from Project 2 single-agent `AgentRole` / `AgentOrchestrator`. |
