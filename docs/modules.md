...
| `llmops` | Project 4 Enterprise LLMOps boundary. Experiment / Run Tracking: `ExperimentRunStore` + `InMemoryExperimentRunStore`. Prompt & Model Registry: `PromptRegistry` / `ModelRegistry` with InMemory adapters. Evaluation Gates / Regression Harness: `EvaluationGateEvaluator` / `RegressionHarness` ports with numeric-metric rules (Default adapters in later tasks). Soft link only: run `params` / `metrics` may link to registry or evaluation aggregates without changing store APIs. Distinct from Project 2 `JobStore` and Project 3 `WorkflowRunId`. Serving and LLMOps Observability remain deferred; no LLM-as-judge; does not bind `ai` LanguageModelProvider. |
