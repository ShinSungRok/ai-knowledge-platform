...
| `infra` | Infrastructure edge for Source-of-Truth persistence and Docker scaffolding. `SqlGateway` / `SqlParameter` / `SqlQueryResult` define the SQL execute contract for repository adapters. `InMemorySqlGateway` validates `knowledge_documents` SQL without Docker/`pg`. Real `pg` driver remains deferred; local Docker helpers via `pnpm infra:config`. |
