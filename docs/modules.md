...
| `infra` | Infrastructure edge for Source-of-Truth persistence and Docker scaffolding. `SqlGateway` / `SqlParameter` / `SqlQueryResult` define the SQL execute contract. `knowledgeSchemaSql` + `applyKnowledgeSchema` apply CREATE TABLE IF NOT EXISTS for sources/documents/chunks. `InMemorySqlGateway` validates repository SQL (DDL no-op) without Docker/`pg`. Real `pg` driver remains deferred; local Docker helpers via `pnpm infra:config`. |
