...
| `http` | Framework-independent HTTP abstraction: `HttpMethod`/`HttpRequest`/`HttpResponse`, `HttpHandler`/`HttpRouter` ports, and `DefaultHttpRouter` (exact method+path match; JSON 404 on miss). Also `ObservingHttpRouter` (logger/metrics decorator). No Express/Fastify; TCP listen lives in `server` (`NodeHttpListener`). |
