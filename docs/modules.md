...
| `composition` | Composition root — wires concrete adapters. In-memory cited-answer + MCP registry/handler; operations/listening expose Bearer-protected HTTP including `POST /mcp`. Optional OTLP when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (`createOperationsObservability`); default sinks stay InMemory. SQL/Postgres paths and OpenSearch remain separate. |
