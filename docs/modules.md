...
| `security` | Authentication and workspace authorization. `Authenticator` / `AuthPrincipal` define AuthN (credentials → principal). `WorkspaceAuthorizer`/`DefaultWorkspaceAuthorizer` and `HttpWorkspaceGuard` (`x-workspace-id`) enforce AuthZ. Health stays unauthenticated. JWT/OIDC SDKs, rate limit, CORS remain deferred. |
