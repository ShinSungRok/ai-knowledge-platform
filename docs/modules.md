...
| `persistence` | Concrete adapters (`DefaultInMemoryRepository`, `DefaultInMemoryKnowledgeSourceRepository`, `DefaultInMemoryDocumentChunkRepository`, `SqlKnowledgeDocumentRepository` over `SqlGateway` for `knowledge_documents`). Real `pg` driver still deferred. |
