...
| `workflow` | Multi-Agent role/identity + Orchestrator + Handoff + Shared Workflow Memory (Project 3). `WorkflowAgent` remains identity-only. `WorkflowHandoff` + builder pass prior output into later steps. `WorkflowMemoryStore` / `InMemoryWorkflowMemoryStore` record objective/handoff/step_output per `workflowRunId` (write-only during orchestration; distinct from Project 2 session `MemoryStore`). Multi-Agent Evaluation remains deferred. |
