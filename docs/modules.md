...
| `llmops` | Project 4 Enterprise LLMOps boundary. Experiment / Run Tracking: `ExperimentRunStore` + `InMemoryExperimentRunStore`. Prompt & Model Registry: `PromptRegistry` / `ModelRegistry` ports with template/version and model/version records (InMemory adapters in later Sprint tasks). Soft link only: run `params` may later store `promptVersionId` / `modelVersionId` without changing ExperimentRunStore. Distinct from Project 2 `JobStore` and Project 3 `WorkflowRunId`. Gates, Serving, and LLMOps Observability remain deferred; does not bind `ai` LanguageModelProvider. |
